...
contract StakingPool is Context, Ownable, ReentrancyGuard {
    ...
    function unstake(uint256 pid, uint256 _amount) external nonReentrant {
        uint256 amount = _amount * pools[pid].fee / FACTOR;
        IERC20(token).safeTransfer(_msgSender(), (_amount - amount));
    ...
    function _removeShares(uint256 pid, address wallet, uint256 amount) private {
        ...
        bool _otherStakersPresent = pools[pid].totalSharesDeposited - amount > 0;
    ...
    function updatePools(uint256 pid, uint256 _lockupSecond, uint256 _percentage, uint256 _rps, uint256 _fee) external onlyOwner {
        require(_lockupSecond < 365 days, "lte 1 year");
        require(_percentage < 10000, "lte 100%");
        require(_fee < 5000, "lte 50%");
        pools[pid].lockupPeriod = _lockupSecond;
        pools[pid].percentage = _percentage;
        pools[pid].rewardsPerShare = _rps;
        pools[pid].fee = _fee;
    }
